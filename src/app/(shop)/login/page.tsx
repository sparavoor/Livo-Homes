      ...
      >
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-secondary/60 text-[10px] uppercase font-black tracking-[0.4em] mt-4">
            {isOtpSent ? 'Verify Identity' : 'Secure Entry'}
          </p>
        </div>
        ...
        <form onSubmit={isOtpSent ? handleVerifyOtp : handleSendOtp} className="space-y-8">
          ...
          >
            {loading ? 'Processing...' : (isOtpSent ? 'Authenticate' : 'Request Access')}
          </button>
        </form>
        ...
        <div className="relative my-12">
          ...
          <div className="relative flex justify-center">
            <span className="px-6 bg-white text-secondary/30 font-black text-[8px] uppercase tracking-[0.4em]">Alternative Portal</span>
          </div>
        </div>
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-background">Initialising...</div>}>
      <LoginContent />
    </Suspense>
  );
}
