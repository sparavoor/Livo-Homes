    ...
    return (
      <div className="min-h-screen bg-background pt-32 px-6 md:px-12 max-w-[1440px] mx-auto animate-pulse">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-20">
          ...
          <div className="lg:col-span-9 h-96 bg-surface-container-low rounded-sm"></div>
        </div>
      </div>
    );
  }
                    ...
                    <div key={order.id} className="border border-outline rounded-sm p-6 sm:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:shadow-lg transition-shadow bg-white">
                      <div className="space-y-3">
                        <div className="flex items-center gap-4">
                          ...
                          <span className={`text-[10px] uppercase tracking-[0.2em] font-medium px-2 py-1 rounded-sm ${order.status === 'delivered' ? 'text-green-700 bg-green-50' :
                ...
                >
                  ...
                  <div className="space-y-12">
                    <div className="space-y-4">
                      <div className="flex items-center gap-4">
                        <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-brand-accent">Order Details</span>
                        <span className={`text-[10px] uppercase tracking-[0.2em] font-medium px-2 py-1 rounded-sm ${selectedOrder.status === 'delivered' ? 'text-green-700 bg-green-50' :
                    ...
                    <div className="pt-12 flex flex-col sm:flex-row gap-6">
                      {selectedOrder.status === 'pending' && (
                        ...
                        >
                          {isCancelling ? 'Processing Withdrawal...' : 'Cancel Order'}
                        </button>
                      )}
                      <button
                        onClick={() => setSelectedOrder(null)}
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <div className="max-w-xl">
                <form onSubmit={handleUpdateProfile} className="space-y-8">
                  ...
                  >
                    {isUpdating ? "Saving Credentials..." : "Save Profile Updates"}
                  </button>
                </form>
              </div>
            </motion.div>
