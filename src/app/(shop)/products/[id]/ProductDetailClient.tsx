...
export default function ProductDetailClient({ product, relatedProducts, categories, subcategories }: ProductDetailClientProps) {
  const { addItem } = useCart();

  const { catId, subId } = useMemo(() => {
    const cat = categories.find(c => c.name === product.category);
    let sId = null;
    if (product.subcategory && cat) {
      const sub = subcategories.find(s => s.name === product.subcategory && s.categoryId === cat.id);
      if (sub) sId = sub.id;
    }
    return { catId: cat?.id || null, subId: sId };
  ...
  return (
    <div className="font-body selection:bg-brand-accent/20">
      {/* Breadcrumbs */}
      <MotionSection>
        <nav className="flex items-center space-x-3 text-xs text-secondary mb-10 md:mb-16 tracking-wide uppercase font-bold">
          ...
          {catId && (
            <>
              <Link className="hover:text-primary transition-colors" href={`/category/${catId}`}>{product.category}</Link>
              <span className="text-outline-variant text-[10px]">/</span>
            </>
          )}
          {subId && catId && (
            <>
              <Link className="hover:text-primary transition-colors" href={`/category/${catId}/${subId}`}>{product.subcategory}</Link>
              <span className="text-outline-variant text-[10px]">/</span>
            </>
          )}
