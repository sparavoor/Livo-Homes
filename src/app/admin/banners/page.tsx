...
export default function AdminBanners() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (fileInputRef.current?.files?.[0] && supabase) {
      ...
      if (uploadError) {
        ...
        // We don't return here, we just continue with imageUrl = image (which is the base64 string)
      } else {
    ...
    try {
      ...
      alert('Save Error: ' + (error.message || 'Unknown error during architectural manifesting.'));
    } finally {
  ...
  async function handleDelete(id: string) {
    try {
      ...
      alert('Architectural asset discarded successfully.');
      loadBanners();
    } catch (error: any) {
              ...
              <div className="relative h-64 overflow-hidden">
                ...
                {banner.image ? (
                  ...
                  <div className="w-full h-full bg-background flex items-center justify-center">
                    <span className="material-symbols-outlined text-primary/10 text-6xl">image</span>
                  </div>
                )}
