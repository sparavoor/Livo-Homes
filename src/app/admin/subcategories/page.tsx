...
import { createSubcategoryAction, deleteSubcategoryAction, fetchSubcategoriesAction, updateSubcategoryAction } from './actions';
...
export default function AdminSubcategories() {
  ...
  async function handleSubmit(formData: FormData) {
    try {
      if (editingSubcategory) {
        await updateSubcategoryAction(editingSubcategory.id, formData);
      } else {
  ...
  async function handleDelete(id: string) {
    if (confirm('Are you sure you want to delete this subcategory? This might affect products using it.')) {
      try {
  ...
  return (
    <div className="flex flex-col gap-10">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h2 className="text-4xl font-headline font-extrabold text-primary tracking-tight">Sub-Collection Registry</h2>
          <p className="text-secondary mt-1 text-sm font-medium opacity-70 uppercase tracking-widest">Specific Architectural Niches</p>
        </div>
        <div className="flex gap-2 bg-surface-container-low p-1 rounded-xl border border-outline-variant/20 shadow-sm">
          ...
          <button 
            onClick={() => { setEditingSubcategory(null); setActiveTab('add'); }} 
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-primary/5 border border-outline-variant/30 overflow-hidden">
          <div className="p-8 border-b border-outline-variant/20 bg-surface-container-low/30">
            <div className="relative max-w-md">
              ...
              <input 
                type="text" 
                placeholder="Find in sub-collections..." 
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-surface-container-low/50 border-b border-outline-variant/20 text-secondary text-[0.65rem] font-bold uppercase tracking-[0.2em]">
                  ...
                  <th className="p-6">Sub-Collection Name</th>
                  <th className="p-6">Parent Collection</th>
              ...
              <tbody className="divide-y divide-outline-variant/10">
                {isLoading ? (
                  ...
                  <tr><td colSpan={6} className="p-20 text-center text-secondary font-medium italic">No sub-collections currently registered.</td></tr>
                ) : subcategories.map((subcategory) => (
                  <tr key={subcategory.id} className="text-sm font-medium hover:bg-primary/[0.02] transition-colors group">
                    <td className="p-6">
                      <div className="w-20 h-20 rounded-xl bg-surface-container-low p-2 shadow-inner border border-outline-variant/10 group-hover:scale-105 transition-transform">
                        ...
                        <img className="w-full h-full object-cover rounded-lg" alt={subcategory.name} src={subcategory.image} />
                      </div>
                    </td>
                    <td className="p-6 text-primary">
                      <div className="font-headline font-bold text-base">{subcategory.name}</div>
                      <div className="text-[10px] text-secondary/60 mt-1 uppercase tracking-tighter">ID: {subcategory.id}</div>
                    </td>
                    <td className="p-6">
                      <span className="px-3 py-1 bg-primary/5 text-primary border border-primary/10 rounded-full text-[10px] font-bold uppercase tracking-wider">
                        {categories.find(c => c.id === subcategory.categoryId)?.name || subcategory.categoryId}
                      </span>
                    </td>
                    <td className="p-6 max-w-xs">
                      <p className="text-secondary text-xs line-clamp-2">{subcategory.description}</p>
                    </td>
                    <td className="p-6">
                    ...
                    <td className="p-6 text-right space-x-6">
                      ...
                      <button className="text-red-500 font-bold text-[10px] uppercase tracking-widest hover:text-red-700 transition-colors" onClick={() => handleDelete(subcategory.id)}>Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-primary/5 border border-outline-variant/30 p-12 max-w-4xl min-h-[500px] flex flex-col">
          <div className="flex justify-between items-center mb-10 pb-6 border-b border-outline-variant/20">
            <h2 className="text-3xl font-headline font-bold text-primary">
              {editingSubcategory ? 'Update Sub-Collection' : 'Establish Sub-Collection'}
            </h2>
            {editingSubcategory && (
              <button 
                onClick={() => { setEditingSubcategory(null); setActiveTab('list'); }}
          ...
          <form action={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8 flex-1" encType="multipart/form-data">
            <div className="md:col-span-2">
              <label className="block font-bold text-primary text-[0.65rem] uppercase tracking-[0.2em] mb-3">Sub-Collection Name</label>
              <input name="name" required type="text" defaultValue={editingSubcategory?.name} placeholder="e.g. Modern Basin Mixers" className="w-full bg-surface-container-low border border-outline-variant/30 focus:ring-2 focus:ring-primary/20 focus:border-primary rounded-xl px-5 py-4 text-sm font-medium transition-all" />
            </div>
            <div className="md:col-span-2">
              <label className="block font-bold text-primary text-[0.65rem] uppercase tracking-[0.2em] mb-3">Parent Collection</label>
              <select name="categoryId" required defaultValue={editingSubcategory?.categoryId} className="w-full bg-surface-container-low border border-outline-variant/30 focus:ring-2 focus:ring-primary/20 focus:border-primary rounded-xl px-5 py-4 text-sm font-bold appearance-none transition-all cursor-pointer">
            ...
            <div className="md:col-span-2 space-y-4">
              <label className="block font-bold text-primary text-[0.65rem] uppercase tracking-[0.2em] mb-3">Cover Image</label>
                ...
                {editingSubcategory && editingSubcategory.image && (
                  <div className="pt-4 border-t border-outline-variant/20">
                    ...
                    <div className="w-24 h-24 rounded-lg bg-white border border-outline-variant/20 p-2 shadow-sm overflow-hidden">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={editingSubcategory.image} alt="Current Asset" className="w-full h-full object-cover" />
                    </div>
                    <input type="hidden" name="existingImage" value={editingSubcategory.image} />
                  </div>
                )}
                
                <div className="space-y-2">
                  <label className="block font-bold text-primary text-[0.65rem] uppercase tracking-[0.2em]">Or provide a master URL</label>
                  <input name="image" type="text" defaultValue={editingSubcategory?.image} placeholder="https://..." className="w-full bg-surface-container-low border border-outline-variant/30 focus:ring-2 focus:ring-primary/20 focus:border-primary rounded-xl px-5 py-4 text-sm font-medium transition-all" />
                </div>
              </div>
            </div>
            <div className="md:col-span-2">
              <label className="block font-bold text-primary text-[0.65rem] uppercase tracking-[0.2em] mb-3">Narrative</label>
              <textarea name="description" required rows={4} defaultValue={editingSubcategory?.description} placeholder="Describe the focus of this sub-collection..." className="w-full bg-surface-container-low border border-outline-variant/30 focus:ring-2 focus:ring-primary/20 focus:border-primary rounded-xl px-5 py-4 text-sm font-medium transition-all" />
            </div>
            <div className="md:col-span-2 pt-8 text-right">
              <button type="submit" className="bg-primary text-white px-12 py-5 rounded-2xl font-bold text-xs uppercase tracking-[0.25em] shadow-2xl shadow-primary/30 hover:scale-[1.02] hover:-translate-y-1 active:scale-95 transition-all">
                {editingSubcategory ? 'Save Changes' : 'Publish Sub-Collection'}
              </button>
            </div>
          </form>
        </div>
