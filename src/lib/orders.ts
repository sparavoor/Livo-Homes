...
export async function getOrdersByContact(contact: { userId?: string; email?: string }) {
  ...
  if (contact.userId && contact.email) {
    query = query.or(`user_id.eq.${contact.userId},customer_email.eq.${contact.email}`);
  } else if (contact.userId) {
    query = query.eq('user_id', contact.userId);
  } else if (contact.email) {
    query = query.eq('customer_email', contact.email);
  } else {
    return [];
  }
...
export async function getUserOrders(userId: string) {
  return await getOrdersByContact({ userId });
}
