  ...
  return (
    <section className="relative h-[95vh] min-h-[750px] w-full overflow-hidden bg-primary px-4 py-4">
      <AnimatePresence mode="wait">
        ...
        >
          {/* Use regular img for external URLs to avoid hostname whitelist issues, next/image for local/base64 */}
          {currentBanner.image ? (
            currentBanner.image.startsWith('http') && !currentBanner.image.includes('supabase.co') && !currentBanner.image.includes('localhost') ? (
              <img 
              ...
              <Image 
                ...
                priority={current === 0}
                sizes="100vw"
                unoptimized={currentBanner.image.startsWith('data:')}
              />
            )
          ) : (
            <div className="w-full h-full bg-primary/20 flex items-center justify-center">
              <span className="material-symbols-outlined text-white/10 text-9xl">landscape</span>
            </div>
          )}
